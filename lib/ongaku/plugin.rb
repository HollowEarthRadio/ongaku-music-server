# File cpied from Redmine Project, before modification
# Origional Copywrite Header:
...
module Ongaku #:nodoc:
  ...
  class Plugin
    ...
    # Sets a requirement on Ongaku version
    # Raises a PluginRequirementError exception if the requirement is not met
    #
    # Examples
    #   # Requires Redmine 0.7.3 or higher
    #   requires_ongaku :version_or_higher => '0.7.3'
    #   requires_ongaku '0.7.3'
    #
    #   # Requires Redmine 0.7.x or higher
    #   requires_ongaku '0.7'
    #
    #   # Requires a specific Redmine version
    #   requires_ongaku :version => '0.7.3'              # 0.7.3 only
    #   requires_ongaku :version => '0.7'                # 0.7.x
    #   requires_ongaku :version => ['0.7.3', '0.8.0']   # 0.7.3 or 0.8.0
    #
    #   # Requires a Redmine version within a range
    #   requires_ongaku :version => '0.7.3'..'0.9.1'     # >= 0.7.3 and <= 0.9.1
    #   requires_ongaku :version => '0.7'..'0.9'         # >= 0.7.x and <= 0.9.x
    def requires_ongaku(arg)
      arg = { :version_or_higher => arg } unless arg.is_a?(Hash)
      arg.assert_valid_keys(:version, :version_or_higher)

      current = Ongaku::VERSION.to_a
      arg.each do |k, req|
        case k
    ...
    #   requires_ongaku_plugin :foo, :version_or_higher => '0.7.3'
    #   requires_ongaku_plugin :foo, '0.7.3'
    #
    #   # Requires a specific version of a Redmine plugin
    #   requires_ongaku_plugin :foo, :version => '0.7.3'              # 0.7.3 only
    #   requires_ongaku_plugin :foo, :version => ['0.7.3', '0.8.0']   # 0.7.3 or 0.8.0
    def requires_ongaku_plugin(plugin_name, arg)
      arg = { :version_or_higher => arg } unless arg.is_a?(Hash)
      arg.assert_valid_keys(:version, :version_or_higher)
